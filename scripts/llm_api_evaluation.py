...
For real MIMIC-derived inputs, use this script only when data-use approval and
the provider contract allow the transfer. Prefer local models for restricted data.
"""
...
def build_prompt(disease: str, row: pd.Series) -> list[dict[str, str]]:
    system = (
        "Clinical diagnostic assistant. Use only the provided structured "
